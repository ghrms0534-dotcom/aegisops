...
export function ProviderSection({ title, kind, names }: { title: string; kind: string; names: string[] }) {
  ...
  const [githubStatus, setGithubStatus] = useState('확인 중');
  ...
  useEffect(() => {
    ...
    if (names.includes('GitHub Actions')) {
      githubApi.getStatus()
        .then(response => {
          setGithubStatus(response.data.status === 'Connected' ? '연결됨' : response.data.status);
          setRepositoryCount(response.data.repositories);
        })
        .catch(() => setGithubStatus('인증 실패'));
    }
    if (names.includes('Jenkins')) {
      Promise.all([jenkinsApi.getStatus(), jenkinsApi.getJobs(), jenkinsApi.getBuilds(), jenkinsApi.getPipelines()])
        .then(([, jobs, builds, pipelines]) => setJenkins({ jobs: jobs.data.jobs, builds: builds.data.builds, pipelines: pipelines.data.pipelines }))
  ...
  return <section><div className="mb-4"><h4 className="font-semibold text-white">{title}</h4><p className="mt-1 text-xs text-brand-muted">{kind} 연결 상태</p></div>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{names.map(name => {
      ...
      const label = isGitHub ? githubStatus : isJenkins ? jenkinsError ? '연결 끊김' : jenkins ? 'Demo Mode' : '확인 중' : error ? '연결 끊김' : !provider ? '확인 중' : provider.summary.mode === 'demo' ? 'Demo Mode' : provider.status === 'ok' ? '연결됨' : provider.status === 'unavailable' ? '연결 끊김' : '연결 준비됨';
      const displayName = name === 'GitHub Actions' ? 'GitHub' : name;
      const badgeClass = label === '연결됨' ? 'badge-success' : label === '연결 끊김' || label === '인증 실패' ? 'badge-danger' : 'badge-warning';
      return <article key={name} className="card flex items-center gap-4 hover:border-blue-500/40"><span className="rounded-lg border border-blue-500/20 bg-blue-500/10 p-2.5 text-blue-400">{kind === 'Cloud Provider' ? <Cloud size={20} /> : label === '연결됨' ? <ShieldCheck size={20} /> : <Plug size={20} />}</span><div className="min-w-0 flex-1"><h5 className="font-medium text-white">{displayName}</h5><p className="mt-1 text-xs text-brand-muted">{providerLabels[name] || kind}</p><p className="mt-2 text-xs text-slate-300">상태: {label}</p>{isGitHub && githubStatus === '연결됨' && <p className="mt-1 text-xs text-slate-300">저장소: {repositoryCount}</p>}{isJenkins && jenkins && <div className="mt-3 space-y-2 border-t border-slate-700/50 pt-3 text-xs text-slate-300"><div><p className="font-medium text-white">Jobs</p>{jenkins.jobs.map(job => <p key={job.name} className="mt-1 break-words">{job.name} · {job.status} · #{job.last_build}</p>)}</div><div><p className="font-medium text-white">최근 Build</p>{jenkins.builds.map(build => <p key={`${build.job}-${build.build_number}`} className="mt-1 break-words">{build.job} #{build.build_number} · {build.result}</p>)}</div><div><p className="font-medium text-white">Pipelines</p>{jenkins.pipelines.map(pipeline => <p key={pipeline.name} className="mt-1 break-words">{pipeline.name}: {pipeline.stages.map(stage => `${stage.name} (${stage.status})`).join(' · ')}</p>)}</div></div>}</div><span className={`badge ${badgeClass}`}>{label}</span></article>;
    })}</div>
  </section>;
}
